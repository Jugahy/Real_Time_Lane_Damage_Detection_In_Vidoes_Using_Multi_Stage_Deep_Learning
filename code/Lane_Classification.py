...
if __name__ == "__main__":
    ...
    # 기본 저장 경로 설정
    setimg = './default_path/setimg2/'  # 이미지 저장 경로
    setimg_m = './default_path/setimg2_mask/'  # 마스크 저장 경로

    track_history = defaultdict(lambda: [])  # 객체 궤적 기록

    # YOLO 모델 로드
    model = YOLO("./default_path/segment_best.pt")  # 세그멘테이션 모델 경로
    names = model.names  # 클래스 이름

    # 분류 모델 로드
    model_classification = torch.load('./default_path/mobilenet_v3_large.pth', map_location=torch.device('cpu'))  # 분류 모델 경로
    classes = ['A', 'B', 'C', 'F']  # 클래스 레이블
    classes_pen = {'A': (0, 255, 0), 'B': (255, 0, 0), 'C': (0, 255, 255), 'F': (0, 0, 255)}  # 클래스별 색상
    model_classification.eval()  # 분류 모델 평가 모드 설정

    # 비디오 파일 경로 설정
    video_file = './default_path/video.mp4'  # 비디오 파일 경로
    ...
    while True:
        ...
        if results[0].boxes.id is not None and results[0].masks is not None:
            clss = results[0].boxes.cls.cpu().tolist()  # 클래스 ID 리스트
            boxes = results[0].boxes.xyxy.cpu().tolist()  # 바운딩 박스 리스트
            masks = results[0].masks.xy  # 마스크 리스트
            track_ids = results[0].boxes.id.int().cpu().tolist()  # 추적 ID 리스트

            for box, mask, track_id, cls in zip(boxes, masks, track_ids, clss):
                if len(mask) == 0:
                    print(box)
                    annotator.box_label(box, color=colors(track_id, True), label=names[int(cls)])  # 바운딩 박스와 클래스 표시
                    continue

                x, y, w1, h1 = int(box[0]), int(box[1]), int(box[2]), int(box[3])  # 바운딩 박스 좌표
                size_p = (w1 - x) * (h1 - y)  # 박스 크기 계산
                center_x = x + int((w1 - x) / 2)  # 중심 x 좌표
                center_y = y + int((h1 - y) / 2)  # 중심 y 좌표
                ...
                try:
                    cv2.fillPoly(mask_image, [mask], (255, 255, 255))  # 마스크 채우기
                except Exception as e:
                    print("마스크 생성 중 오류:", e)

                size_seg = int(np.sum(mask_image) / 255)  # 마스크 크기 계산

                # 마스크 및 객체 크기 기준 필터링
                if size_p >= size_pixel and size_seg >= size_seg_pixel:
                    if save_img:
                        cv2.imwrite(setimg + names[int(cls)] + '_id' + str(track_id) + ".png", frame)
                        print(f"{setimg}{names[int(cls)]}_id{track_id}.png 저장 완료!")
                    if view_img:
